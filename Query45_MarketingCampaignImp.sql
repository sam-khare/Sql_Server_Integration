...
/*next day after purchase doesn't count*/
WITH CTE as
(
Select *,rank() over (partition by user_id order by created_at ) as rnum,
lag(created_at,1) over (partition by user_id order by user_id) as lag_date
...
SELECT *
from CTE 
WHERE (DATEADD(day,1,lag_date) <> created_at)
and created_at <> lag_date
...
from cte
where rnum = 1
)c3 on c2.user_id = c3.user_id and c2.product_id = c3.product_id 

WHERE c3.user_id is null

--------------------------------Query when just day doesn't count-------------
...
SELECT c3.user_id
from cte3 c3 left join cte2 c2 on c3.user_id = c2.user_id and c3.product_id = c2.product_id
